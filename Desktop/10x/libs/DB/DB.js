...
class DB {
  ...
  */
   ...
   *  @param { (hashKey: string) => string } [options.hashCB]  - функция для использования кастомной функции на hash keys 
   * @return { dbItemsObject<T> } A DbItemsObject containing the generated data items.
   **/
  getDataItemsObject(useKeys, { filterCB = undefined, itemCB = undefined, keyCB = undefined, ignoreKeys = undefined, hashCB = undefined } = {}) {
    const keys = ignoreKeys === undefined ? this.keys : this._filterKeysFrom([...[ignoreKeys ?? []].flat()]);
    const values = this.values;
    const dbItems = this.dbItems;
    const items = new DBItemsObject();
    const vl = values.length
    const hashFunction = hashCB === undefined ? this._getKeysHashFunction(useKeys) : hashCB;
  ...
  */
   ...
   *   @param { (currentItem: dbItem<T> , newItem: dataItem<T> ) => item } [options.hashCB]  - функция для использования кастомной функции на hash keys 
   ...
   */
  mergeDataItems(dataItems, useKeys, { isUpdateCB = undefined, updateCB = undefined, isDeleteCB = undefined, filterCB = undefined, keyCB = undefined, ignoreKeys = undefined, hashCB = undefined } = {}) {

    const currentItemsObj = this._parseDataItemsArrayToObject(this.getDataItemsArray({ filterCB, ignoreKeys }), useKeys, keyCB, hashCB)
    ...
    //@ts-ignore
    const dataItemsObj = this._checkDataItems(dataItems) === "object" ? dataItems : this._parseDataItemsArrayToObject(dataItems, useKeys, keyCB, hashCB);
  ...
  */
   ...
   * @param { (hashKey: string) => string } [keyCB] - функция преобразования ключа
   @param { (hashKey: string) => string } [hashCB]  - функция для использования кастомной функции на hash keys
   * @returns { dbItemsObject < dbItem<T> > }
   */
  _parseDataItemsArrayToObject(dataItems, useKeys, keyCB = undefined, hashCB = undefined) {
    const hashFunction = hashCB === undefined ? this._getKeysHashFunction(useKeys) : hashCB;
  ...
  */
   ...
   */
  checkSheet() {
    const sheet = DB._getSheet(this.sheetName, this.ssId)
  ...
  createSheet() {
    ...
    const keysRow = [];
    Object.entries(this.keys).forEach((key, column) => {
      keysRow[column] = key;
    })
    newSheet.getRange(this.keysRow, keysRow.length).setValues([keysRow]);
    newSheet.deleteColumns(keysRow.length + 1, newSheet.getMaxColumns() - keysRow.length + 1);
    newSheet.deleteRows(this.dataRowFirst + 1, newSheet.getMaxRows() - this.dataRowFirst + 2);
    this.setFormats();
    const sheet = DB._getSheet(this.sheetName, this.ssId);
    if (!sheet) throw Error(`[DB][${this.ssId}][${this.sheetName}]: sheet creation error`)
  ...
  /**
   * @param { dbParams } params - параметры
   * @returns { DB }
  */
  static init({
    sheetName,
    ...
    return new DB({ sheetName, ssId, keysRow, dataRowFirst, dataColumnFirst, headersRow, dataColumnsNum, keys, formats })
  }
}
 ...
 * @property { string } [ssId = "activeSpreadsheet"] - ssId таблицы
 * @property { number } [keysRow = 1] - номер строки с ключами
 * @property { number } [headersRow] - номер строки с заголовками
 * @property { number } [dataRowFirst = 2] - номер первой строки с данными
 * @property { number } [dataColumnFirst = 1] - номер первой колонки с данными
 * @property { number } [dataColumnsNum] - количество колонок с данными
 * @property { string[] | keys} [keys] - объект | массив с ключами
 * @property { { [format:string]: string} } [formats]
*/

/** 
  * @tempalate T 
  * @param { Object } params - параметры
  * @param { string } params.sheetName - название листа
  * @param { string } [params.ssId = "activeSpreadsheet"] - ssId таблицы
  * @param { number } [params.keysRow = 1] - номер строки с ключами
  * @param { number } [params.headersRow] - номер строки с заголовками
  * @param { number } [params.dataRowFirst = 2] - номер первой строки с данными
  * @param { number } [params.dataColumnFirst = 1] - номер первой колонки с данными
  * @param { number } [params.dataColumnsNum] - количество колонок с данными
  * @param { string[] | keys} [params.keys] - объект | массив с ключами
  * @param { { [format:string]: string} } [params.formats]
  * @returns { DB<T> }
  */
...
function init({
  ...
  return DB.init({ sheetName, ssId, keysRow, dataRowFirst, dataColumnFirst, headersRow, dataColumnsNum, keys, formats })
}
